...
+udev +webrtc-aec +X zeroconf pali-patches pali-legacy-hsp pali-aptx"
...
REQUIRED_USE="
	...
	bluetooth? ( dbus )
	equalizer? ( dbus )
	ofono-headset? ( bluetooth !pali-patches )
	native-headset? ( bluetooth !pali-patches )
	realtime? ( dbus )
	udev? ( || ( alsa oss ) )
	pali-patches? ( bluetooth !ofono-headset !native-headset )
	pali-legacy-hsp? ( pali-patches )
	pali-aptx? ( pali-patches )
"
...
RDEPEND="
	...
	pali-aptx? ( >=media-libs/libopenaptx-0.2.1 )
...
RDEPEND="${RDEPEND}
	...
	acct-group/audio
	pali-patches? ( media-sound/hsphfpd )
"
...
src_prepare() {
	...
	use pali-patches && ( eapply "${FILESDIR}/pulseaudio-pali.patch" || die )
...
multilib_src_configure() {
	...
	if use bluetooth; then
		# pali-patches and {ofono,native}-headset
		# are mutually exclusive
		if use pali-patches; then
			myconf+=(
				$(multilib_native_use_enable pali-legacy-hsp bluez5-legacy-hsp)
				$(multilib_native_use_enable pali-aptx aptx)
			)
		else
			myconf+=(
...
pkg_postinst() {
	...
	if use pali-patches; then
		elog "You have enabled Pali's PulseAudio patchset. This disables ofono support and"
		elog "replaces it with hsphfpd. It should be pulled in as a dependency."
	fi
}
