...
# based on wine-vanilla-9999.ebuild
...
# we could use proton_7.0 or experimental_7.0
# as this should be used for running games and
# nothing else, we choose the experimental branch
if [[ ${PV} == *9999 ]]; then
	SRC_URI=""
	inherit git-r3
...
# removed from below "kerberos", add back when proton updates to fixed wine
...
src_prepare() {
	...
	local geckomono=$(sed -En '/^#define (GECKO|MONO)_VER/{s/[^0-9.]//gp}' \
		"${S}/dlls/appwiz.cpl/addons.c" || die)
	...
	# generate vulkan headers (proton build handles this regularly)
	dlls/winevulkan/make_vulkan -x vk.xml || die
}
...
src_configure() {
	...
	local conf=(
		...
		# wine bug, compilation fails, fixed in 7.13
		#$(use_with ldap)
		--without-ldap
		# TODO: Will bug 685172 still need special handling?$(use_with mingw)
		$(use_with netapi)
