...
# jovaska: don't care about native languages, feel free to enable them again if you want to
MOZ_LANGS=(
	en-US
)
...
src_prepare() {
	...
	if use icecat-patches; then
		local temp

		# apply patches
		while read temp; do
			eapply "$temp"
		done < <( find "${FILESDIR}/icecat/patches/" -type f -name '*.patch' )

		# append files
		#while read temp; do
		#	# l10n modifications not possible ATM
		#	[[ "$temp" == ./l10n/* ]] && continue
        #	einfo "Appending new data to the end of file: $temp"
		#	cat "${FILESDIR}/icecat/files-to-append/${temp}" >> "${S}/${temp}" || die 
		#done < <( cd "${FILESDIR}/icecat/files-to-append"; find . -type f -print )

		# l10n stuff (does not work)
		#local PREFS_IN_FILE=browser/locales/en-US/browser/preferences/preferences.ftl
		#local PREFS_OUT_FILE=/browser/browser/preferences/preferences.ftl
		#while read temp; do
		#	# this is a bit confusing...
		#	if [[ "$temp" != ./l10n/compare-locales ]]; then
		#		if [[ ! -e "${FILESDIR}/icecat/files-to-append/${temp}/${PREFS_OUT_FILE}" ]]; then
		#			cat "${FILESDIR}/icecat/files-to-append/${PREFS_IN_FILE}" \
		#				>> "${S}/${temp}/${PREFS_OUT_FILE}" || die
		#		fi
		#	fi
	...
	if use extra-patches; then
		...
		# these still exist, so just get rid of them
		rm -rf "${S}/browser/components/search/extensions/"* || die

		# new v2 search config
		rm -rf "${S}/services/settings/dumps/main/search-config-icons" || die
		cp -r "${FILESDIR}/search/search-config-icons" \
			"${S}/services/settings/dumps/main" || die
		sed "s/@TS@/${ts}/g" "${FILESDIR}/search/search-config-v2.json" \
			> "${S}/services/settings/dumps/main/search-config-v2.json" || die
		sed "s/@TS@/${ts}/g" "${FILESDIR}/search/search-config-icons.json" \
			> "${S}/services/settings/dumps/main/search-config-icons.json" || die
		...
		sed "s/@TS@/${ts}/g" "${FILESDIR}/search/empty.json" \
			> "${S}/services/settings/dumps/main/search-config-overrides.json" || die
		sed "s/@TS@/${ts}/g" "${FILESDIR}/search/empty.json" \
			> "${S}/services/settings/dumps/main/search-config-overrides-v2.json" || die

		# need to modify moz.build to add custom icons
		local temp path
		while read path; do
			[[ "$temp" ]] && temp="${temp},\\n"
			temp="${temp}\\1\\1\"${path}\""
		done < <( cd "${FILESDIR}/search"; find search-config-icons -type f | sort )
		sed -zi 's@\([^\S\n]*\)\(FINAL_TARGET_FILES\.defaults\.settings\.main\["search-config-icons"\] += \)\[[^]]\+\]@\1\2[\n'${temp}'\n\1]@g' \
			"${S}/services/settings/dumps/main/moz.build" || die
	...
	# jovaska: clear all vendor checksums because I'm not going to
	# manually keep track of what all the patches modify
	local crate
	while read crate; do
		crate="${crate##*/}"
...
src_install() {
	...
	# jovaska: custom tweaks
	if use extra-patches; then
		# install enterprise policy
		newins "${FILESDIR}/jovaska-default-policy.json" policies.json
		insinto "${MOZILLA_FIVE_HOME}"

		# get config filename from autoconfig.js
		local name="$(grep 'general\.config\.filename' "${FILESDIR}/jovaska-autoconfig.js" \
			| cut -d, -f2 | grep -o '"[^"]\+"' | cut -d\" -f2)"

		# construct autoconfig.js
		echo "// autoconfig.js generated on $(date --rfc-email)"$'\n' > "${T}/autoconfig.js"
		cat "${FILESDIR}/jovaska-autoconfig.js" >> "${T}/autoconfig.js"
		use userchrome-js && echo "pref(\"general.config.sandbox_enabled\", false);" >> "${T}/autoconfig.js"

		# construct config file
		echo "// ${name} generated on $(date --rfc-email)"$'\n' > "${T}/${name}"
		cat "${FILESDIR}/jovaska-default-prefs.js" >> "${T}/${name}"
		if use userchrome-js; then
			echo >> "${T}/${name}"
			cat "${FILESDIR}/userchrome-js-loader.js" >> "${T}/${name}"
		fi
