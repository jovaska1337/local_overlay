...
# FIXME: langpack selection is broken --> fix when merging with makeicecat
...
mozilla_set_globals() {
	...
	for lang in "${MOZ_LANGS[@]}" ; do
		...
		# icecat tarball icludes all langpacks by default
		IUSE+=" l10n_${xflag/[_@]/-}"
	done
}
mozilla_set_globals
...
pkg_nofetch() {
	# FIXME: this nonsense should be merged into a single ebuild
...
src_unpack() {
	# FIXME: again, merge makeicecat to this ebuild
	if use buildtarball; then
		unpack "/usr/src/makeicecat-${PV}/output/icecat-${PV}-gnu1.tar.bz2" \
			|| eerror "Tarball is missing, check that www-client/makeicecat has use flag buildtarball enabled."
	else
