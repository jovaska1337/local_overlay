...
MOZ_ICECAT_FILE="icecat"

if [[ ! -r ${MOZ_LIB_DIR}/icecat/${MOZ_ICECAT_FILE} ]]; then
	if [[ ! -r ${SECONDARY_LIB_DIR}/icecat/${MOZ_ICECAT_FILE} ]]; then
		echo "Error: ${MOZ_LIB_DIR}/icecat/${MOZ_ICECAT_FILE} not found" >&2
		if [[ -d ${SECONDARY_LIB_DIR} ]]; then
			echo "       ${SECONDARY_LIB_DIR}/icecat/${MOZ_ICECAT_FILE} not found" >&2
		fi
		exit 1
	fi
...
MOZ_EXTENSIONS_PROFILE_DIR="${HOME}/.icecat/extensions/{ec8030f7-c20a-464f-9b0e-13a3a9e97384}"
MOZ_PROGRAM="${MOZILLA_FIVE_HOME}/${MOZ_ICECAT_FILE}"
