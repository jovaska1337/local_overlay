...
# because this shit is necessary for some fucking reason
ARCHIVES="${FILESDIR}/archives"
...
src_unpack() {
	...
	# we use local copies, as adobe doesn't host the files anymore
	if use nsplugin; then
		use abi_x86_32 && files+=( "${ARCHIVES}/${P}-npapi.i386.tar.gz" )
		use abi_x86_64 && files+=( "${ARCHIVES}/${P}-npapi.x86_64.tar.gz" )
	fi
	if use ppapi; then
		use abi_x86_32 && files+=( "${ARCHIVES}/${P}-ppapi.i386.tar.gz" )
		use abi_x86_64 && files+=( "${ARCHIVES}/${P}-ppapi.x86_64.tar.gz" )
	fi
